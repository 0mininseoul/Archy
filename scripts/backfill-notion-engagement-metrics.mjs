...
async function main() {
  ...
  for (let current = startDate; current <= endDate; current = addDaysYmd(current, 1)) {
    ...
    if (!apply) {
      printLine(
        `${current} ${label} -> 이용률(누적) ${usageRate}, 활성화율(30일) ${activationRate30d}, MAU ${metrics.counts.mau}명, 녹음 횟수 ${metrics.counts.dailyRecordings}회, 녹음 유저 ${metrics.counts.dailyRecordingUsers}명`
      );
      continue;
    }
