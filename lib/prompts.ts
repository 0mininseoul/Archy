...
- **제목은 반드시 30자 이내**로 작성하세요. UI에서 최대 2줄까지만 표시되므로, 짧고 핵심적인 제목이어야 합니다.
- 녹취록 첫 문장 복사 금지, "짧은 음성 메모" 같은 일반적인 제목 금지

## 본문 작성 규칙
1. 맨 앞에 **✅ 핵심 요약** 섹션을 두세요 (제목에 체크 이모지 포함: `## ✅ 핵심 요약`).
...
## 서식 규칙
- **LaTeX 문법 사용 금지**: \`$\\rightarrow$\`, \`$\\times$\` 등 LaTeX 수식을 절대 사용하지 마세요. 대신 유니코드 기호를 직접 사용하세요 (→, ×, ÷, ≥, ≤ 등).
- **이탤릭체(*기울임*) 사용 금지**: 용어 강조에 이탤릭체를 사용하지 마세요. 강조가 필요하면 **볼드체**를 사용하세요.
