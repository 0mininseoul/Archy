...
아래에 제공된 녹취록은 내용이 짧습니다. 있는 내용만 간결하게 정리해주세요.
...
2. **"녹취록을 제공해주세요", "내용을 입력해주세요" 등의 응답을 하지 마세요.** 위에 녹취록이 이미 있습니다.
3. 내용이 부족해도 억지로 채우지 마세요.
4. 불명확한 부분은 그대로 두거나 "[불명확]"으로 표시하세요.

## 요구사항
- 짧은 내용이므로 1-2줄 핵심만 정리하세요
- 내용이 너무 짧으면 "녹음 내용이 짧아 요약이 제한적입니다"라고 명시하되, 있는 내용은 반드시 포함하세요
...
⚠️ 주의: "(제목)", "(있는 내용만 정리)" 같은 플레이스홀더나 입력 요청 문구를 절대 출력하지 마세요.`;

// Minimal Prompt - 매우 짧은 녹취록용 (15단어 미만)
const MINIMAL_PROMPT = `당신은 전문 에디터입니다.
아래에 제공된 녹취록은 매우 짧습니다. 있는 내용 그대로만 정리해주세요.
...
- **"녹취록을 제공해주세요", "내용을 입력해주세요" 등의 응답을 하지 마세요.** 위에 녹취록이 이미 있습니다.
- 녹취록에 있는 것만 그대로 정리하세요
- 내용이 부족해도 만들어내지 마세요

## 출력 형식 (반드시 이 형식대로 출력하세요)
[TITLE]
녹취록 내용을 간단히 요약한 짧은 제목 (예: "짧은 메모", "간단한 녹음")
[/TITLE]
[CONTENT]
...
⚠️ 주의: "(짧은 제목)" 같은 플레이스홀더나 입력 요청 문구를 절대 출력하지 마세요. 반드시 위 녹취록을 기반으로 작성하세요.`;
