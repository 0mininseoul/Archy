...
3. **"녹음 내용이 짧습니다"라고만 말하고 끝내지 마세요.** 반드시 녹취록의 실제 내용을 포함해야 합니다.

## 요구사항
- 녹취록 내용을 기반으로 제목을 작성하세요
- 핵심 내용을 1-2줄로 요약하세요
- 짧더라도 녹취록에 있는 내용은 반드시 본문에 포함하세요

## 출력 형식 (반드시 따르세요)
[TITLE]
녹취록 핵심을 담은 제목 (예: "테스트 중 발생한 오류 관련")
[/TITLE]
[CONTENT]
### 📌 핵심 내용
- (녹취록의 실제 핵심 내용을 작성)
...
⚠️ 중요: 반드시 녹취록의 실제 내용을 포함해서 출력하세요.`;
...
## ⚠️ 절대 금지 사항
- **녹취록을 그대로 복사하여 붙여넣기 금지** - 반드시 요약/정리된 형태로 작성
- **"내용이 짧습니다", "내용이 부족합니다" 등의 메타 코멘트 금지**
- **"녹취록을 제공해주세요" 같은 입력 요청 금지**
- **녹취록에 없는 내용 추가 금지**

## 필수 요구사항
1. 녹취록 내용을 기반으로 핵심을 담은 제목 작성
2. 녹취록 내용을 **요약하여** 본문 작성 (원본 그대로 복사 금지)
...
⚠️ 중요: 녹취록 원본을 그대로 붙여넣지 말고, 반드시 요약/정리된 형태로 작성하세요.`;
